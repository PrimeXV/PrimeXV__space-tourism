...
const technologyPortraitImages = [
...
const technologyLandscapeImages = [
...
const getBackgroundImages = () => {
  if (window.matchMedia("(max-width: 781px)").matches) {
    return bgImagesMobile;
  } else if (window.matchMedia("(max-width: 1024px)").matches) {
    return bgImagesTablet;
  } else {
    return bgImagesDesktop;
  }
};
...
navLinks.addEventListener("click", (e) => {
  ...
  document.querySelector(`.section--${clicked.dataset.section}`).style.display =
    "block";
...
window.addEventListener("resize", () => {
  if (window.matchMedia("(max-width: 781px)").matches) {
    document.querySelector(".tech-img img").src = technologyLandscapeImages[0];
  } else if (window.matchMedia("(max-width: 1024px)").matches) {
    document.querySelector(".tech-img img").src = technologyLandscapeImages[0];
  } else {
    document.querySelector(".tech-img img").src = technologyPortraitImages[0];
  }
...
destinationNavLinks.forEach((item, i) => {
  item.addEventListener("click", (e) => {
    ...
    columnImg.src = destinationImages[i];
  });
});
...
const slider = () => {
  ...
  dotContainer.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("dots__dot")) {
      ...
      sliderImg.src = crewImages[+slide];
    }
  });
};
slider();
...
const getScreenSizeImages = () => {
  if (window.matchMedia("(max-width: 780px)").matches) {
    return technologyLandscapeImages;
  } else if (window.matchMedia("(max-width: 1024px)").matches) {
    return technologyLandscapeImages;
  } else {
    return technologyPortraitImages;
  }
};
//   const images = getScreenSizeImages();
//   techImg.src = images[index]; // Update this index based on your logic
// };
